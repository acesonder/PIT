...
CREATE TABLE IF NOT EXISTS pit_assessments (
    ...
    assessment_type ENUM('SHORT', 'MEDIUM', 'HARD') NOT NULL,
...
('Short Assessments', 'SELECT COUNT(*) as value FROM pit_assessments WHERE assessment_type = "SHORT" AND is_complete = TRUE', 'Number of short assessments completed', FALSE, 3),
('Medium Assessments', 'SELECT COUNT(*) as value FROM pit_assessments WHERE assessment_type = "MEDIUM" AND is_complete = TRUE', 'Number of medium assessments completed', FALSE, 4),
('Hard Assessments', 'SELECT COUNT(*) as value FROM pit_assessments WHERE assessment_type = "HARD" AND is_complete = TRUE', 'Number of hard assessments completed', FALSE, 5),
('Full Consent Count', 'SELECT COUNT(*) as value FROM consent_records WHERE full_consent = TRUE', 'Individuals who gave full consent', FALSE, 6),
('Partial Consent Count', 'SELECT COUNT(*) as value FROM consent_records WHERE partial_consent = TRUE', 'Individuals who gave partial consent', FALSE, 7),
('Assessments Today', 'SELECT COUNT(*) as value FROM pit_assessments WHERE DATE(created_at) = CURDATE()', 'Assessments completed today', TRUE, 8),
('Assessments This Week', 'SELECT COUNT(*) as value FROM pit_assessments WHERE YEARWEEK(created_at) = YEARWEEK(NOW())', 'Assessments completed this week', FALSE, 9),
('Assessments This Month', 'SELECT COUNT(*) as value FROM pit_assessments WHERE YEAR(created_at) = YEAR(NOW()) AND MONTH(created_at) = MONTH(NOW())', 'Assessments completed this month', FALSE, 10),
('Active Staff', 'SELECT COUNT(*) as value FROM outreach_staff WHERE is_active = TRUE', 'Number of active outreach staff', FALSE, 11),
('Average Assessment Duration', 'SELECT "N/A" as value', 'Average time to complete assessment', FALSE, 12),
('Male Identified', 'SELECT COUNT(*) as value FROM pit_assessments WHERE JSON_EXTRACT(assessment_data, "$.gender") = "Man"', 'Individuals identifying as male', FALSE, 13),
('Female Identified', 'SELECT COUNT(*) as value FROM pit_assessments WHERE JSON_EXTRACT(assessment_data, "$.gender") = "Woman"', 'Individuals identifying as female', FALSE, 14),
('Non-Binary', 'SELECT COUNT(*) as value FROM pit_assessments WHERE JSON_EXTRACT(assessment_data, "$.gender") IN ("Non-binary", "Two-Spirit")', 'Non-binary and Two-Spirit individuals', FALSE, 15),
('Indigenous Identity', 'SELECT COUNT(*) as value FROM pit_assessments WHERE JSON_EXTRACT(assessment_data, "$.indigenous") = "Yes"', 'Individuals identifying as Indigenous', FALSE, 16),
('Veterans', 'SELECT COUNT(*) as value FROM pit_assessments WHERE JSON_EXTRACT(assessment_data, "$.veteran") = "Yes"', 'Veterans counted', FALSE, 17),
('Sheltered', 'SELECT COUNT(*) as value FROM pit_assessments WHERE JSON_EXTRACT(assessment_data, "$.location_type") = "Shelter"', 'Staying in shelters', FALSE, 18),
('Unsheltered', 'SELECT COUNT(*) as value FROM pit_assessments WHERE JSON_EXTRACT(assessment_data, "$.location_type") IN ("Outside", "Vehicle", "Encampment")', 'Unsheltered individuals', FALSE, 19),
